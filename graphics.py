        ...
        def function1():
            ...
            def next1_command():
                if val != '' and  val.title() in country_list:
                    ...
                    new1 = 1
                    new2 = 1
                    wikiurl = "https://en.wikipedia.org/wiki/"+val
                    weatherurl = "https://www.timeanddate.com/weather/"+val
                    mapsurl = "https://www.google.com/maps/place/"+val

                    def openwiki():
                        webbrowser.open(wikiurl,new=new)

                    def openweather():
                        webbrowser.open(weatherurl, new = new1)

                    def openmaps():
                        webbrowser.open(mapsurl, new = new2)

                    wikibutton = Button(canvas2, text = 'WIKI INFO', height = 5, width = 20, bg = 'black', fg = 'white', command = openwiki, font = buttonfont1, relief = 'groove')
                    wikibutton.pack(side = TOP)

                    weatherbutton = Button(canvas2, text = 'WEATHER', height = 5, width = 20, bg = 'black', fg = 'white', command = openweather, font = buttonfont1, relief = 'groove')
                    weatherbutton.pack(side = TOP)

                    mapsbutton = Button(canvas2, text = 'MAPS', height = 5, width = 20, bg = 'black', fg = 'white', command = openmaps, font = buttonfont1, relief = 'groove')
                    mapsbutton.pack(side = TOP)
                    ...
                    countryimg = None
                    try:
                        countryimg = ImageTk.PhotoImage(Image.open("images\\000001.png"))
                    except:
                        countryimg = ImageTk.PhotoImage(Image.open("images\\000001.jpg"))
                    mylabel = Label(canvas1)
                    canvas1.create_image(400, 400, anchor = CENTER, image = countryimg)

                    try:
                        os.remove("images\\000001.png")
                    except:
